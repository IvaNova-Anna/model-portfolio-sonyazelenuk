...
test('отвергает VP8X с поврежденными битами', () => {
  ...
  buf[20] = 0xf0; // Зарезервированные биты (4-7) должны быть 0, но здесь они 1 (флаги-байт при смещении 20)
  buf.writeUIntLE(1066, 24, 3); // ширина-1 (24 бита при смещениях 24-26)
  buf.writeUIntLE(1599, 27, 3); // высота-1 (24 бита при смещениях 27-29)
...
test('build() включает имя файла в ошибку', () => {
  ...
  try {
    ...
    buf.write('VP8X', 12);
    buf.writeUInt32LE(10, 16); // Корректный размер чанка: 10 байт
    buf[20] = 0xf0; // Зарезервированные биты должны быть 0 (флаги-байт при смещении 20)
